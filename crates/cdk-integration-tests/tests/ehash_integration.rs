...
use cdk::mint::{MintBuilder, MintMeltLimits, MintQuoteRequest, MintQuoteResponse};
...
async fn wait_for_paid(mint: &Mint, quote_id: QuoteId) {
    ...
    loop {
        ...
        let responses = mint
            ...
            .await
            .expect("check_mint_quotes");

        if let Some(MintQuoteResponse::Custom { response, .. }) = responses.first() {
...
async fn test_multiple_quotes_can_be_paid_independently() {
    ...
    let states1 = mint
        .check_mint_quotes(std::slice::from_ref(&id1))
        .await
        .expect("check 1");
    if let Some(MintQuoteResponse::Custom { response, .. }) = states1.first() {
        assert_eq!(
            response.state,
            QuoteState::Unpaid,
...
async fn test_pay_unknown_hash_does_not_affect_existing_quotes() {
    ...
    let states = mint
        ...
        .await
        .expect("check");
    if let Some(MintQuoteResponse::Custom { response, .. }) = states.first() {
        assert_eq!(
            response.state,
            QuoteState::Unpaid,
