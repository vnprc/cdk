...
use cdk_common::mint::{BatchMintRequest, IncomingPayment, MintQuote, Operation};
...
impl Mint {
    ...
    pub async fn process_mint_request(
        ...
        let result = async {
            ...
            let endpoint_payment_method = self
                .localstore
                .get_mint_quote(&quote)
                .await?
                .map(|mint_quote| mint_quote.payment_method)
                .ok_or(Error::UnknownQuote)?;
    ...
    async fn process_mint_workload(
        ...
        for quote in &quotes {
            let quote_amount = match payment_method {
                ...
                PaymentMethod::MiningShare => quote.amount.ok_or(Error::AmountUndefined)?,
